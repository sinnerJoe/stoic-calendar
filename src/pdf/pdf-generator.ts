...
const SQUARE_SIZE =
  (PAPER_WIDTH -
    PADDING * 2 -
    SPACE_BETWEEN_SQUARES * (SQUARES_PER_LINE - 1) -
    MIDDLE_GAP) /
  SQUARES_PER_LINE;
...
class PDFGenerator {
  ...
  renderFooter() {
    ...
    this.setDefaultFont();
    this.doc.text(
      `*Cross out one square on each ${this.birthDayOfWeek}.`,
      xPos,
      this.y + PADDING / 2,
  ...
  drawYear() {
    const calculateX = (column) =>
      PADDING + column * SQUARE_SIZE + (column - 1) * SPACE_BETWEEN_SQUARES;
