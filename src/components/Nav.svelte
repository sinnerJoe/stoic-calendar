<script lang="ts">
  ...
  function visitPDFUrl() {
    generatePaper($birthDate, $expectancy).save(
      `${formattedDate}_${$expectancy}.pdf`
    );
  }
...
<nav>
  <div class="space-between">
    <div class="controls">
      <label class="space-right datepicker-container" for="date">
        <span>Birth</span>
        <DatePicker
          bind:visible
          {startDate}
...
<AgeModal
  bind:visible={visibleAgeModal}
  on:change={modalChangeExpectancy}
  value={$expectancy}
/>
<FloatingMenu
  on:clickBirthEntry={openDatePicker}
  on:clickDownload={visitPDFUrl}
  on:clickExpectancy={openAgePicker}
/>
