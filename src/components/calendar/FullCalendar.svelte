<script lang="ts">
  import { differenceInWeeks, addYears, isLeapYear } from "date-fns";
  ...
  // the calendar displays only the number of weeks per non-leap year
  // so we remove extra weeks formed from leap years
  $: weeksUncounted = countLeapWeeksBetween(birth);

  $: weeksConsumed = differenceInWeeks(new Date(), birth);
  $: weeksTotal = differenceInWeeks(addYears(birth, years), birth);
  $: {
    let weeksLived = weeksConsumed - weeksUncounted;
    function didExpire() {
...
<div>
  <p>
    You have lived <b>{weeksConsumed}</b> weeks out of <b>{weeksTotal}</b> given
    to you, <b>{weeksTotal - weeksConsumed}</b> weeks remaining.
  </p>
